...
const VALID_TABS = ["ai-inbox", "messages", "live-chat"] as const;
type Tab = typeof VALID_TABS[number];

export default function CommsHub() {
  const search = useSearch();
  const raw = new URLSearchParams(search).get("tab") ?? "";
  const tab: Tab = (VALID_TABS as readonly string[]).includes(raw) ? (raw as Tab) : "ai-inbox";
  ...
  return (
    <Tabs value={tab} onValueChange={goTab} className="space-y-4">
      <TabsList className="flex-wrap h-auto gap-1">
        <TabsTrigger value="ai-inbox" data-testid="tab-comms-ai-inbox">🤖 AI Inbox</TabsTrigger>
      ...
      <TabsContent value="ai-inbox"><AiInbox /></TabsContent>
