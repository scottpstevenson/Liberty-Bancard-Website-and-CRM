...
**Corrective patch round (4 defects fixed in the same task, no new follow-ups):** crash-resumption in `previewStagingV2` misclassifying already-committed same-commandKey rows as blocked (fixed via a `resumeCommandKey` param reconstructing the original 'eligible' preview row from the existing intent's snapshot); the worker and `executeStagingV2` each creating their own ledger run for the same batch (fixed via a `stageRunId` passthrough); the plaintext boundary being escapable through `openSfpCandidatePlaintext` callbacks (fixed with a runtime `assertNoPlaintextEscape()` guard); and operator UI/telemetry/runbook gaps for the existing retry/cancel routes (added UI wiring, `workerHealth`/backlog-ETA telemetry, and rewrote the runbook sections that still described the old unbuilt state).

## Disposable-DB certification fixture gotchas (this domain)
- A cohort-member row for any business a fixture will reference must be inserted BEFORE the cohort's `frozen` UPDATE — a DB trigger (`sfp_reject_frozen_member_mutation`) rejects `sfp_cohort_members` inserts once `cohort_state` is frozen/voided/superseded. Add new fixture businesses to the pre-freeze batch loop, not after it.
- Rejected/blocked eligibility rows (mismatch, drift, etc.) never get `staging_intent_id` set, so they remain visible to the recurring worker's "unstaged eligible" query indefinitely — a batch-limited worker tick can silently consume its whole batch on older leftover fixture rows before ever reaching the row a later assertion expects, making that assertion see zero items for that business. Assert the invariant (e.g. "no business processed by this run has more than one item row") over whatever the run actually touched, not a specific hardcoded business.
- The worker's default program lookup falls back to a hardcoded name (`'south-florida-v1'` here) when no `programId` is passed — a fixture program must use that exact name for a no-arg recurring-tick call to find it, or the tick silently reports `enabled: false`.
