...
/**
 * Load operator-verified pricing from mi09_pricing_artifacts (latest per
 * provider). Delegates selection ("latest artifact per provider") and field
 * shape to `buildCro03PriceScheduleFromArtifacts()` — the SAME shared,
 * dependency-free helper the server-side MI-09 pricing snapshot service
 * (`createPricingScheduleSnapshot()` in mi09-pilot-authority.ts) uses, so
 * this script and that service can never independently drift on what
 * counts as "latest" or on the composite-hash input shape.
 */
async function loadPricingFromArtifacts(
  ...
  const { buildCro03PriceScheduleFromArtifacts } = await import("../server/services/cro03/contracts");
  try {
    const { schedule } = buildCro03PriceScheduleFromArtifacts(artifacts);
    return schedule as unknown as PriceSchedule;
  } catch (error: any) {
    const message = String(error?.message ?? error);
    const missing = message.startsWith("CRO03_PRICING_ARTIFACTS_MISSING:")
      ? message.split(":")[1]
      : message;
    throw new Error(
      `Missing pricing artifacts for providers: ${missing}. ` +
