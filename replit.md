...
- **Automated Residual Reconciliation**: Admin/manager can upload monthly processor residual reports (CSV or XLSX) from the Residual Revenue page. The system parses the file, matches rows to existing merchant records by MID, calculates expected vs. actual variance, flags out-of-range items (configurable % and $ thresholds), and shows per-agent reconciliation. One-click confirm posts residuals to agent ledgers and triggers an internal variance alert notification. Full import history is viewable by month. New tables: `residual_imports`, `residual_import_rows`. New API endpoints: `POST /api/residuals/import`, `GET /api/residuals/imports`, `GET /api/residuals/imports/:id`, `POST /api/residuals/imports/:id/confirm`, `DELETE /api/residuals/imports/:id`.
- **Direct Processor Boarding & MID Data Pipeline**: Full NMI-style processor boarding integration for the onboarding pipeline. New schema fields on `deals`: `processorApplicationId`, `mid`, `boardingStatus` (not_submitted/submitted/under_review/more_info_needed/approved/declined), `boardingLog` (JSONB event timeline), `boardingSubmittedAt`, `boardingApprovedAt`. New `mid_daily_stats` table stores per-MID daily volume, txCount, avgTicket, effectiveRate, chargebackCount, chargebackAmount, refundCount. Processor API service (`server/services/processor-api.ts`) runs in simulation mode when `PROCESSOR_API_KEY`/`PROCESSOR_API_BASE_URL` env vars are absent. Boarding routes at `server/routes/boarding.ts`: `POST /api/deals/:id/submit-to-processor`, `POST /api/deals/:id/refresh-boarding-status`, `GET /api/deals/:id/boarding-status`, `GET /api/deals/:id/mid-stats`, `POST /api/deals/:id/refresh-mid-stats`, `POST /api/admin/run-mid-ingestion`. SLA worker runs nightly MID data ingestion for all active MIDs. Frontend: `BoardingPanel` component embedded in each deal card on the Deals tab (shows for onboarding pipeline or underwriting/approved stages) with submit button, status badge, and event timeline. `LiveProcessingTab` component on a new "Live Processing" contact detail tab shows 30-day volume trend area chart, 4 KPI cards (volume, transactions, avg ticket, chargebacks), daily breakdown table, and data freshness indicator.
