/**
 * MI-02: DBPR Barbers Adapter
 * Source: myfloridalicense.com/barbers/public-records/
 * Key: dbpr_bar_license_number
 * Filters: explicit barber shop establishment allowlist only (NOT individual barbers)
 */
...
/**
 * EXPLICIT ALLOWLIST — barber shop / school establishment license types only.
 * Any license type NOT in this set is skipped (fail-closed for unknown types).
 */
const BAR_ESTABLISHMENT_ALLOWLIST = new Set([
  "Barber Shop",
  "Barbershop",
  "Barber School",
  "Barber College",
...
export const dbprBarAdapter: SourceAdapter = {
  adapterKey: "dbpr-bar",
  sourceName: "DBPR Barbers Establishments",
  ...
  normalize(rawRow: Record<string, string>): NormalizedSourceRecord | null {
    ...
    if (!BAR_ESTABLISHMENT_ALLOWLIST.has(licenseType)) return null;
    ...
    const businessName = (rawRow["BusinessName"] || rawRow["Business Name"] || rawRow["ShopName"] || rawRow["Shop Name"] || "").trim() || null;
