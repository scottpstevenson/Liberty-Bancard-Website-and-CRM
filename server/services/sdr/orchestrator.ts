...
async function checkAndHandleOrphanDeal(lead: SdrLeadState, reviewMode: boolean): Promise<void> {
  // TASK-1830: source-staged deal creation fail-closed until Gen-2 crosswalk authorization.
  // The audit-log write is awaited before return so a write failure never allows deal creation.
  await storage.createAuditLog({
    action: "sdr_orphan_deal_fail_closed",
    entityType: "contact",
    entityId: lead.contactId ?? 0,
    details: {
      leadId: lead.id,
      reason: "Gen-1 identity crosswalk active — automated deal creation disabled until Gen-2 authorization",
    },
  });
  return;
  // ── existing body below is unreachable until the guard above is removed ──
