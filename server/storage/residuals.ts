...
import { minorToCurrency, parseCurrencyToMinor } from "../services/money";
import {
  ...
  async generatePayoutsForMonth(month: string) {
    ...
    return db.transaction(async (tx) => {
      ...
      for (const row of residualRows) {
        ...
        const existing = byAgentPartner.get(key) ?? {
          agentId: row.agentId, partnerOrgId, grossResidual: 0n, agentCommission: 0n, partnerCommission: 0n,
        };
        try {
          // Do not silently round legacy text into an authoritative payout.
          existing.grossResidual += parseCurrencyToMinor(row.revenue || "0");
          existing.agentCommission += parseCurrencyToMinor(row.agentCommission || "0");
          existing.partnerCommission += parseCurrencyToMinor(row.partnerCommission || "0");
        } catch {
          continue;
        }
      ...
      for (const [, totals] of byAgentPartner) {
        ...
        const grossResidual = minorToCurrency(totals.grossResidual);
        const agentShare = minorToCurrency(totals.agentCommission);
        const partnerShare = minorToCurrency(totals.partnerCommission);
