...
export function registerReconciliationRoutes(app: Express): void {
  ...
  // ── POST /api/admin/reconciliation/runs/:runId/force-cancel ──────────────
  ...
  app.post("/api/admin/reconciliation/runs/:runId/force-cancel", requireRole("admin"), async (req, res) => {
    const runId = validateRunId(req.params.runId, res);
    if (!runId) return;
    try {
      const r = await pool.query(
      ...
      if (r.rows.length === 0) {
        // Distinguish: does the run exist at all?
        const check = await pool.query(
          `SELECT status, updated_at FROM contact_reconciliation_runs WHERE id=$1`,
          [runId],
        );
      ...
      serverError(res, err, "reconciliation force-cancel run");
    }
  });
